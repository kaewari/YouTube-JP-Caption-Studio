#!/usr/bin/env bash
# deploy-ipad.sh — PC/Mac code change → fresh binary on real iPad ASAP.
...
#
# Signing-only refresh: renew-ipad-signing.sh. For code updates use this script.
#
# Wireless: pair once (USB or Xcode → Devices), then same Wi‑Fi + unlocked iPad;
...
#
# Example alias:
#   alias ipad-deploy='DEVICE_UDID=YOUR-UDID-HERE ~/…/ipad-app/scripts/deploy-ipad.sh'
#
# Env:
...
LOG="${LOG:-${HOME}/Library/Logs/YouTubeJPCaptionStudio-deploy.log}"
