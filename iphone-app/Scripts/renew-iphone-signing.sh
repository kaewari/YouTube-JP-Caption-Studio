#!/usr/bin/env bash
# Renew free Apple Development signing for YouTube JP Caption Studio on a paired iPhone.
# Overwrites the same Bundle ID install — never deletes the app (sandbox data stays).
# For code updates use deploy-iphone.sh instead.
#
# UDID:
...
#   DEVICE_UDID=xxxxxxxx-…          # iPhone UDID
#   TEAM=XXXXXXXXXX                 # Apple Development Team ID (or DEVELOPMENT_TEAM)
# Optional:
#   APP_PATH=/path/to/YouTubeJPCaptionStudioiPhone.app
...
LOG="${HOME}/Library/Logs/YouTubeJPCaptionStudio-iPhone-renew.log"
