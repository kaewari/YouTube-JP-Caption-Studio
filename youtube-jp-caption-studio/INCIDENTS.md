...
3. **App-full: related / masthead / below đè lên video**  
   Cause: `#movie_player { z-index: max }` vẫn thua `#secondary` — z-index của player bị kẹt trong stacking context `#primary`, sibling `#secondary` (sticky related) paint đè lên. Masthead + `#below` cũng còn hiện.  
   Fix: dưới `html.cs-app-full` only — `display:none` `#masthead-container` / `#secondary` / `#related` / `#below`; player + video `background:#000`. LayoutSmoke cho phép scoped hide (cấm unscoped). Không đụng watch layout khi không full.
